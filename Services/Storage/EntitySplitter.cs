...
///
/// Vendored from CIPP's CIPP.TableClient, itself derived from AzBobbyTables
/// (MIT-licensed, © Björn Sundling). The only change from the upstream copy is the
/// marker property names, which carry a "_Craft" prefix here so they cannot collide
/// with the orchestrator Results table's own hand-rolled chunking scheme (which reuses
/// the plain "OriginalEntityId"/"PartIndex" names). Craft's orchestrator tables are
/// private to Craft, so the on-disk format does not need to match CIPP's.
/// </summary>
public static class EntitySplitter
{
    /// <summary>Property holding the JSON manifest describing column-split properties.</summary>
    public const string SplitOverPropsKey = "_CraftSplitOverProps";

    /// <summary>Property on part rows holding the RowKey of the original entity.</summary>
    public const string OriginalEntityIdKey = "_CraftPartOf";

    /// <summary>Property on part rows holding the zero-based row index.</summary>
    public const string PartIndexKey = "_CraftPartIndex";
    ...
    /// </summary>
    public const string PartCountKey = "_CraftPartCount";
