...
public sealed class CraftRoles
{
    ...
    public static CraftRoles Resolve(CraftSettings settings, Func<string, string?> env)
    {
        ...
        var cacheEnabled = (EnvFlag.Read(env, "CRAFT_RESPONSE_CACHE") ?? settings.Cache.Enabled)
                           ?? (frontend && http);

        var healthEnabled = EnvFlag.Read(env, "CRAFT_HEALTH_ENABLED") ?? settings.Health.Enabled;

        var healthPathEnv = env("CRAFT_HEALTH_PATH");
        var healthPath = !string.IsNullOrWhiteSpace(healthPathEnv)
            ? healthPathEnv.Trim()
            : settings.Health.Path;
