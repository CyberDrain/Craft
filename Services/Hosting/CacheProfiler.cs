...
///
/// Segments per cacheable (List* GET) request: auth (the x-ms-client-principal middleware) → userKey
/// (GetUserKey: base64 decode + JSON parse + SHA256) → keyBuild (BuildCacheKey) → get (whole
...
internal static class CacheProfiler
{
    ...
    private static long _count, _auth, _userKey, _keyBuild, _get, _getDisk, _set, _hits, _misses, _memHits;
    private static long _lc, _la, _luk, _lkb, _lg, _lgd, _ls, _lh, _lm, _lmh;
    ...
    internal static void RecordRequest(long userKeyTicks, long keyBuildTicks, long getTicks, bool hit)
    {
        Interlocked.Add(ref _userKey, userKeyTicks);
    ...
    {
        long c = n, a = _auth, uk = _userKey, kb = _keyBuild, g = _get, gd = _getDisk, s = _set, h = _hits, m = _misses, mh = _memHits;
        long dn = c - _lc;
        if (dn <= 0) return;
        double Avg(long cur, long last) => (cur - last) * 1_000_000.0 / Stopwatch.Frequency / dn;
        _logger?.LogWarning(
            "[CacheProfile] window={Dn} hits={H} (mem={Mh}) misses={M} us/req: auth={A:F1} userKey={Uk:F1} keyBuild={Kb:F1} get={G:F1} (disk={Gd:F1}) set={S:F1}",
            dn, h - _lh, mh - _lmh, m - _lm, Avg(a, _la), Avg(uk, _luk), Avg(kb, _lkb), Avg(g, _lg), Avg(gd, _lgd), Avg(s, _ls));
        _lc = c; _la = a; _luk = uk; _lkb = kb; _lg = g; _lgd = gd; _ls = s; _lh = h; _lm = m; _lmh = mh;
    }
}
