...
internal static class HealthEndpoint
{
        ...
        this WebApplication app, CraftRoles roles, StorageHealthMonitor? storageHealth, ILogger logger)
    {
        ...
        var pool = app.Services.GetService<PowerShellWorkerPool>();
        ...
        {
            var httpReady = !roles.Http || (pool?.IsReady ?? false);
            var bgReady = !roles.Background || (pool?.BackgroundReady ?? false);
