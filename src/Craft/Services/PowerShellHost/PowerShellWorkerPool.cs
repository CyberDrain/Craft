...
public class PowerShellWorkerPool : IDisposable
{
    ...
    private readonly Lazy<WorkerMetricsService> _metrics;
    ...
    public PowerShellWorkerPool(
        ScriptRepository repo,
        ILogger<PowerShellWorkerPool> logger,
        CraftSettings settings,
        StartupProgressService startup,
        Lazy<WorkerMetricsService> metrics)
    {
        _repo = repo;
        _logger = logger;
        _settings = settings;
    ...
    public void Initialize(bool enableHttp = true, bool enableBg = true)
    {
        ...
        _startup.SetCpuCount(Environment.ProcessorCount);
        _startup.SetPoolConfig(enableHttp ? _httpPoolSize : 0, enableBg ? _bgPoolSize : 0);
    ...
    {
        _startup.SetModuleCounts(sharedModules.Count, httpOnlyModules.Count, bgOnlyModules.Count);
        ...
        _startup.SetBaseWorkerDone(baseMs, baseState.Functions.Count);
        ...
        var afterReady = IsAfterReadyWarmup(warmupMode);
        var httpWarmup = !IsBackgroundWarmup(warmupMode);

        // BeforeReady: warmup before the ready signal. AfterReady: signal first, then warmup
        // with the worker briefly out of the pool. Background: warmup on the BG first worker.
        if (httpWarmup && !afterReady)
        ...
        _httpReady.Set();
        _ready.Set();
        _startup.SetHttpReady(sw.ElapsedMilliseconds, _httpClonedState.Functions.Count);
        ...
        if (_httpPoolSize > 1)
        {
            ...
            _startup.SetHttpPoolFull(sw.ElapsedMilliseconds);
        ...
        if (IsBackgroundWarmup(warmupMode))
            RunWarmup(firstBgWorker, sw);

        AddToBgPool(firstBgWorker);

        _startup.SetBgReady(sw.ElapsedMilliseconds, _bgClonedState.Functions.Count);
        ...
        _startup.SetFullyReady(sw.ElapsedMilliseconds);
        ...
        bool enableHttp, bool enableBg)
    {
        ...
        var isBgWarmup = IsBackgroundWarmup(warmupMode);
        var afterReady = IsAfterReadyWarmup(warmupMode);
        ...
        if (enableHttp)
        {
            ...
            _startup.SetHttpReady(firstWorkerMs, _httpClonedState.Functions.Count);
    ...
    private void RunWarmup(PowerShellWorker worker, System.Diagnostics.Stopwatch sw)
    {
        ...
        _startup.SetWarmupDone(warmSw.ElapsedMilliseconds);
    }

    /// <summary>
    /// AfterReady: pull the first HTTP worker out of the pool, warm it, put it back.
    /// Requests may already be accepted (<see cref="_httpReady"/> is set); during warmup the
    /// pool has one fewer available worker.
    /// </summary>
    private void RunWarmupFromHttpPool(System.Diagnostics.Stopwatch sw)
    {
        if (_settings.Worker.WarmupScripts.Count == 0) return;
        if (!_httpPool.TryTake(out var worker, 0))
        {
            _logger.LogWarning("[System] AfterReady warmup skipped — no HTTP worker available in pool");
            return;
        }

        try { RunWarmup(worker, sw); }
        finally { _httpPool.Add(worker); }
    }

    private static bool IsBackgroundWarmup(string warmupMode) =>
        warmupMode.Equals("Background", StringComparison.OrdinalIgnoreCase);

    private static bool IsAfterReadyWarmup(string warmupMode) =>
        warmupMode.Equals("AfterReady", StringComparison.OrdinalIgnoreCase);

    /// <summary>
    /// Pre-register all pool workers with WorkerMetricsService so they appear
    /// in snapshots even before their first checkout.
    /// </summary>
    private void RegisterAllWorkers()
    {
        foreach (var id in _httpWorkerIds)
            _metrics.Value.RegisterWorker(id, isHttp: true);
        foreach (var id in _bgWorkerIds)
            _metrics.Value.RegisterWorker(id, isHttp: false);
    }
    ...
    public PowerShellWorker? CheckoutHttp(TimeSpan timeout)
    {
        ...
        if (_httpPool.TryTake(out var w, timeout))
        {
            ...
            _metrics.Value.RecordCheckout(w.Id, isHttp: true);
    ...
    public PowerShellWorker CheckoutBackground(CancellationToken ct)
    {
        ...
        w.CheckoutTimestamp = System.Diagnostics.Stopwatch.GetTimestamp();
        _metrics.Value.RecordCheckout(w.Id, isHttp: false);
    ...
    public void Reclaim(PowerShellWorker worker, bool isHttp, bool faulted = false)
    {
        ...
        var elapsedMs = worker.CheckoutTimestamp > 0
            ...
            : 0;
        _metrics.Value.RecordReclaim(worker.Id, faulted, elapsedMs);
        ...
        {
            var oldId = worker.Id;
            _metrics.Value.DeregisterWorker(oldId);
            worker.Dispose();
            ...
            _ = Task.Run(() =>
            {
                try
                {
                    ...
                    _metrics.Value.RegisterWorker(fresh.Id, ish);
