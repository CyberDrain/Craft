...
public class PowerShellRunnerService : IDisposable
{
    ...
    // Lazy to avoid a ctor cycle with OrchestratorService / QueueDispatchService.
    private readonly Lazy<OrchestratorService> _orchestrator;
    private readonly Lazy<QueueDispatchService> _queueDispatch;

    /// <summary>Fire-and-forget drain tasks still in flight (shutdown flush awaits these).</summary>
    private readonly ConcurrentDictionary<Task, byte> _backgroundDrains = new();
    ...
    public PowerShellRunnerService(
        ...
        WorkerMetricsService metrics,
        Lazy<OrchestratorService> orchestrator,
        Lazy<QueueDispatchService> queueDispatch)
    {
        _logger = logger;
        _pool = pool;
        _repo = repo;
    ...
    public async Task<ScriptResult> ExecuteHttpEndpoint(string endpoint, Hashtable request)
    {
        ...
        try
        {
            ...
            _metrics.RecordFunction(worker.Id, endpoint);
        ...
        DispatchTiming? timing = null)
    {
        ...
        try
        {
            ...
            _metrics.RecordFunction(worker.Id, entry.FunctionName);
    ...
    /// <summary>
    /// Drain pending orchestrator/queue triggers off the calling thread. Ingress queues are
    /// thread-safe, so multiple in-flight drain calls are fine — each TryDequeue serialises.
    /// </summary>
    private void DrainBridgesInBackground()
    {
        var task = Task.Run(async () =>
        {
            ...
            {
                _logger.LogError(ex, "[Scheduler] Background pending-work drain failed");
            }
        });
        TrackBackgroundDrain(task);
    }

    private void TrackBackgroundDrain(Task task)
    {
        _backgroundDrains[task] = 0;
        _ = task.ContinueWith(
    ...
    private async Task DrainPendingWorkAsync()
    {
        await _orchestrator.Value.DrainPendingAsync();
        _queueDispatch.Value.DrainPending();
    }

    /// <summary>
    /// Await in-flight background drains started by this runner. Used on host shutdown after a
    /// final ingress drain. Returns the count still incomplete after <paramref name="timeout"/>.
    /// </summary>
    internal async Task<int> FlushBackgroundDrainsAsync(TimeSpan timeout, CancellationToken cancellationToken)
    {
        var pending = _backgroundDrains.Keys.ToArray();
        ...
        if (finished == all)
        {
            await all; // observe faults
            return 0;
        }

        var leftovers = _backgroundDrains.Count;
        if (leftovers > 0)
            _logger.LogWarning(
                "[Shutdown] {Count} background pending-work drain(s) still running after {Timeout}",
                leftovers, timeout);
        return leftovers;
    }
    ...
    public async Task ExecuteScript(string functionName, Dictionary<string, object>? parameters = null)
    {
        ...
        _metrics.RecordFunction(worker.Id, parentFunction ?? functionName);
    ...
    public void Dispose()
    {
        // Pool lifetime is owned by the DI container (PowerShellWorkerPool is a separate singleton).
        // Disposing it here tore down workers still used by orchestrator/jobs/dispatch on runner teardown.
