...
/// </summary>
public sealed class AzureTableStore : ICraftTableStore, IUserTableStore
{
    ...
    /// <summary>
    /// Shared host store — orchestrator tables and health probes. Never applies
    /// <c>Auth:UserStorageConnection</c>; that override is only for <see cref="IUserTableStore"/>.
    /// </summary>
    public AzureTableStore(CraftSettings settings)
        : this(settings, explicitConnection: null, purpose: "table storage")
    {
    }

    /// <summary>
    /// Store bound to an explicit connection (e.g. allowedUsers isolation). Empty/null
    /// <paramref name="explicitConnection"/> falls through to the shared resolution chain.
    /// </summary>
    public AzureTableStore(CraftSettings settings, string? explicitConnection, string purpose)
    {
        // Resolved lazily so constructing the store on a role that never touches storage does not
        // require a connection string — it is only resolved on first actual use.
        var overrideConnection = string.IsNullOrWhiteSpace(explicitConnection) ? null : explicitConnection;
        _connectionString = new Lazy<string>(() =>
            settings.Storage.ResolveConnection(overrideConnection, purpose));
