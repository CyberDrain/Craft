...
public class JobManager : BackgroundService
{
    ...
    // ── Descriptor rehydration + durability (Lazy avoids ctor cycle with OrchestratorService) ──
    private readonly Lazy<JobWorkResolver>? _resolver;
    private readonly Lazy<IJobDescriptorStateWriter>? _stateWriter;
    ...
    public JobManager(
        ...
        Lazy<IJobDescriptorStateWriter>? stateWriter = null)
    {
        _logger = logger;
        _limiter = limiter;
        _resolver = workResolver;
    ...
    internal List<JobRecord> GetJobs(string? runName = null, string? status = null, int? limit = null)
    {
    ...
    {
        IJobDescriptorStateWriter writer;
        try
        {
            if (_stateWriter is null) return;
            writer = _stateWriter.Value;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[JobManager] Failed to resolve state writer for {What} on {Name}", what, jobName);
            return;
        }
