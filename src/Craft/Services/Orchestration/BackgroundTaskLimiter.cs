...
public class BackgroundTaskLimiter : IDisposable
{
    ...
    public BackgroundTaskLimiter(ILogger<BackgroundTaskLimiter> logger,
        CraftSettings settings, PowerShellWorkerPool pool)
    {
        _logger = logger;
        _pool = pool;

        var limiter = settings.BackgroundLimiter;
        ...
        _overSubscribe = Math.Max(0, limiter.OverSubscribe);
