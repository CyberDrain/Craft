...
public class OrchestratorService : IJobDescriptorStateWriter
{
    ...
    // In-process ingress queues (PowerShell enqueues via OrchestratorBridge; C# drains here).
    private readonly ConcurrentQueue<PendingOrchestration> _pending = new();
    private readonly ConcurrentQueue<PendingPlannerRun> _pendingPlanners = new();

    /// <summary>Fire-and-forget planner/finalize tasks still in flight (shutdown flush awaits these).</summary>
    ...
    /// <summary>
    /// Queue an orchestrator run from PowerShell (via <c>OrchestratorBridge</c>). Drained by
    /// <see cref="DrainPendingAsync"/> after the invoking script returns.
    /// </summary>
    public void QueueOrchestration(string name, string batchJson, int priority,
        string? postExecFunctionName = null, string? postExecParametersJson = null,
        string? parentRunName = null, string? reference = null)
    {
        _pending.Enqueue(new PendingOrchestration(name, batchJson, priority,
            postExecFunctionName, postExecParametersJson, parentRunName, reference));
    }

    /// <summary>
    /// Queue a planner-based orchestrator run. The planner script builds the task list on a
    /// background worker, then tasks dispatch — same path as the scheduler.
    /// </summary>
    public void QueuePlannerRun(string command, int priority) =>
        _pendingPlanners.Enqueue(new PendingPlannerRun(command, priority));
    ...
    /// Thin sync wrapper over <see cref="DrainPendingAsync"/> for PowerShell bridge callers.
    /// </summary>
    public void DrainPending() =>
        DrainPendingAsync().ConfigureAwait(false).GetAwaiter().GetResult();
    ...
    public async Task DrainPendingAsync()
    {
        while (_pending.TryDequeue(out var p))
        {
            try
            {
                await StartFromBatchAsync(p.Name, p.BatchJson, p.Priority,
                ...
                if (!string.IsNullOrEmpty(p.ParentRunName))
                    TryRegisterChildRun(p.ParentRunName, p.Name);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Orchestrator] DrainPending failed for {Name}", p.Name);
            }
        }
        DrainPendingPlanners();
    }

    private void DrainPendingPlanners()
    {
        while (_pendingPlanners.TryDequeue(out var p))
        {
            ...
            async Task RunAsync()
            {
                try { await StartPlannerRunAsync(command, priority, CancellationToken.None); }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "[Orchestrator] Planner run failed for {Command}", command);
                }
            }
            TrackBackgroundWork(RunAsync());
        }
    }

    private void TrackBackgroundWork(Task task)
    {
        _backgroundWork[task] = 0;
        _ = task.ContinueWith(
    ...
    /// <summary>
    /// Await in-flight planner/finalize work. Used on host shutdown after a final ingress drain.
    /// Returns the count still incomplete after <paramref name="timeout"/>.
    /// </summary>
    internal async Task<int> FlushBackgroundWorkAsync(TimeSpan timeout, CancellationToken cancellationToken)
    {
        var pending = _backgroundWork.Keys.ToArray();
        ...
        if (finished == all)
        {
            ...
            {
                _logger.LogError(ex, "[Shutdown] Background orchestrator work faulted during flush");
            }
            return 0;
        }

        var leftovers = _backgroundWork.Count;
        if (leftovers > 0)
            _logger.LogWarning(
                "[Shutdown] {Count} orchestrator background task(s) still running after {Timeout}",
                leftovers, timeout);
        return leftovers;
    }
    ...
    /// </summary>
    /// <summary>
    /// Rehydrates a queued descriptor into runnable work at dispatch time. Registered with
    /// <see cref="JobManager"/> via DI as <see cref="JobWorkResolver"/>.
    /// </summary>
    internal async Task<Func<CancellationToken, Task>?> ResolveTaskWorkAsync(JobDescriptor descriptor, CancellationToken ct)
    {
