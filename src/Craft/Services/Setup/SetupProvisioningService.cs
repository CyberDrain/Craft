...
/// <summary>
/// Handles the first-run bootstrap provisioning:
///   1. Device-code token exchange (auth code → access token, no client secret)
...
/// </summary>
public class SetupProvisioningService
{
    private readonly ILogger<SetupProvisioningService> _logger;
    ...
    static SetupProvisioningService()
    {
    ...
    public SetupProvisioningService(ILogger<SetupProvisioningService> logger, CraftSettings settings)
    {
        _logger = logger;
        _settings = settings;
    }
    ...
    /// </summary>
    public async Task<SetupService.DeviceCodeResponse> StartDeviceCodeFlow(CancellationToken ct = default)
    {
        ...
        return new SetupService.DeviceCodeResponse
        {
    ...
    /// </summary>
    public async Task<SetupService.TokenExchangeResult?> PollDeviceCodeFlow(string deviceCode, CancellationToken ct = default)
    {
        ...
        return new SetupService.TokenExchangeResult
        {
    ...
    /// </summary>
    public async Task<SetupService.AppRegistrationResult> CreateAuthAppRegistration(
        string accessToken, string tenantId, string redirectUri, bool multiTenant = false, CancellationToken ct = default)
    {
        ...
        return new SetupService.AppRegistrationResult
        {
            AppId = appId,
    ...
    {
        if (!SetupSessionState.IsEasyAuthConfigured())
        {
